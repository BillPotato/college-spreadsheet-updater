...
class college_sheet:
    ...
    def __init__(self, sheet_id):
        ...
        self.spreadsheet_obj = ezsheets.Spreadsheet(sheet_id)[0]
        self.rows: list = self.spreadsheet_obj.getRows()
        self.college_name_column = self.spreadsheet_obj.getColumn(1) # don't remove header
        self.key_columns: dict[int, str] = {}
        self.unfilled_colleges: list[int] = []
        self.college_threads: list = []
...
#         print(f"Fetching page for {self.name}...")
        ...
        for unfilled_cell in self.unfilled_cells:
            ...
            try:
                ...
                self.row[unfilled_cell] = f"{Error}: Could not find school! (check name)"
            # Error handler
